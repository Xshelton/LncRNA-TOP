...
for index_mp in range(0,len(list_of_mp)):
 ...
 #constructed_LC(refine_493)_rs{}_mkPCA{}mer_mkPCA{}.csv
 #constructed_ILC+IncTar(refine_1382)_rs_1_4096.csv
 #constructed_ILC+IncTar(refine_1382)_rs_{}_{}.csv
 for rs in range(0,10):
    for k in range(1,2):
            try:
             print('try to load dataset for training')
             X_train,y_train,X_test,y_test=file_to_dataset('.\\Results\\temp\\constructed_ILC+IncTar(refine_1382)_rs_{}_{}.csv'.format(frs,mp),'lnc','gene',0.1+0.1*k,rs)
             ...
             X_train,y_train,X_test,y_test=embedding2dataset(fileA='.\\lncRNA\lncRNA_features\\mix PCA\\lnc-palindormic_mix_kernalpolyPCA{}).csv'.format(mp),#lnc-palindormic_kernelpolyPCA64).csv
                                                               labelA='label',
                                                               fileB='.\\gene\\gene_features\\mix kpca\\gene mix_kernelpolyPCA{}).csv'.format(gp),
                                                               labelB='label',
                                                               dataset='.\\Dataset construction\\manuelly curated negative samples\\constructed_LC+IncTar(refine_1382)_rs{}.csv'.format(frs),
                                                             #
                                                               dataA='lnc',
                                                               dataB='gene',
                                                               outputname='.\\Results\\temp\\constructed_ILC+IncTar(refine_1382)_rs_{}_{}.csv'.format(frs,mp),
                                                               partion=0.1+0.1*k,
                                                               randomseed=rs,
                                                               )
 ...
 lrf.to_csv('.\\Results\\{}_constructed_LC+IncTar(refine_1382)_rs{}_(poly+poly)_Both_mix_kpca{}_{}portion_fix{}_{}_auc{}aupr_{}_epoch{}.csv'.format(lname,frs,mp,gp,(1-0.1-0.1*k),'',m1,m2,rs+1))
